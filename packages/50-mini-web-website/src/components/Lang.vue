...
<template>
    <div class="mini-web-tab-container">
        <div class="mini-web-tab-item" v-for="(item, index) in state.tabList" :key="index + item.value"
            v-text="item.label" @click="onTabClickHandler(item.value)"
            :class="{ active: item.value === state.currentLang }"></div>
    </div>
</template>
