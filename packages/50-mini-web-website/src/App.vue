      ...
      <main class="mini-web-main">
         <header class="mini-web-header">
            ...
            <async-select class="mini-web-header-select" v-model="webProjectType">
               <async-option v-for="(type, index) in typeList" :key="type.value + index" :value="type.value"
                  :label="type.label">{{ type.label }}</async-option>
            </async-select>
         </header>
         <div class="mini-web-main-content">
            <async-card v-for="(card, index) in newDataList" :key="card.name + index" :cardSourceURL="card.source"
               :href="card.href" :directory="directory" :cardName="card.name" :cardIndex="index"></async-card>
         </div>
