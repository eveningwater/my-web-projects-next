 ...
 * github:https://github.com/eveningwater/my-web-projects/tree/master/js/9
 * author:eveningwater
 * date:2020/7
 */
...
function addTransaction() {
  if (!text.value.trim() || !amount.value.trim()) {
    return ewMessage.error('请填写交易内容(收入来源)与交易金额(收入金额)');
  } else {
    let transaction = {
      id: getRandomId(),
      text: text.value,
...
function addTransactionDOM(transaction) {
  ...
  item.innerHTML = `
        ...
        <button type="button" class="delete-btn" onclick="removeTransaction(${
          transaction.id
        })">&times;</button>
    `;
...
function updateValues() {
  ...
  // 消费
  const cost = (
    amounts.filter(amount => amount < 0).reduce((c, v) => (c += v), 0) * -1
  ).toFixed(2);
